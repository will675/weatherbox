...
class BootProvisioner:
    ...
    def __init__(self, credential_file: str = "/etc/weatherbox/credentials.yaml"):
        """
    ...
    def provision(self) -> bool:
        ...
        if credentials:
            ...
            if self._attempt_connection(ssid, password):
                ...
                logger.warning("Failed to connect to stored network; falling back to AP mode")
        else:
            logger.info("No stored credentials found; starting AP for provisioning")
    ...
    def _attempt_connection(self, ssid: str, password: str, max_attempts: int = 3) -> bool:
        """
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                if self.wifi_adapter.connect(ssid, password, timeout_seconds=30):
                    logger.info(f"Connected to {ssid} on attempt {attempt}")
                    return True
                else:
                    logger.warning(f"Connection attempt {attempt} failed")
            except Exception as e:
                logger.error(f"Connection attempt {attempt} raised exception: {e}")
...
def main():
    ...
    # Configure logging
    configure_logging(log_level="INFO", log_file="/var/log/weatherbox/provisioning.log")
    ...
    try:
        ...
        if success:
            ...
            if provisioner.logged_in:
                ...
                logger.info("Access point is ready for provisioning at http://192.168.4.1:8080")
            
            # Start provisioning service/display service would happen here
            # For now, just log success
            return 0
        else:
            logger.error("Boot provisioning failed; device may not be operable")
            return 1
        ...
        logger.error(f"Unexpected error during boot provisioning: {e}", exc_info=True)
        return 1
