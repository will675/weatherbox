...
def create_app(credential_store=None, wifi_adapter=None) -> Flask:
    ...
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'weatherbox-provisioning-dev-key')
    app.config['SESSION_COOKIE_SECURE'] = False  # Disabled for local network access
    ...
    def initialize_session():
        ...
        if 'csrf_token' not in session:
            session['csrf_token'] = serializer.dumps({'nonce': os.urandom(16).hex()})
    ...
    def provision():
        ...
        if len(ssid) > 32:
            ...
            return jsonify({'error': 'SSID must be 32 characters or less'}), 400
        
        if len(password) > 63:
            logger.warning(f"Password too long: {len(password)} characters")
            return jsonify({'error': 'Password must be 63 characters or less'}), 400
        ...
        try:
            if not app.credential_store:
                ...
                return jsonify({'error': 'Credential storage not available'}), 500
