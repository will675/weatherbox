    ...
    def get_instance(cls):
        ...
        if cls._instance is None:
            try:
                ...
                logger.warning(
                    "rgbmatrix library not available; using mock mode")
                cls._instance = None
...
class RpiAdapter:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if rgbmatrix_class is None:
                logger.warning(
                    "rpi-rgb-led-matrix not installed; using mock mode")
    ...
    def render_frame(self, matrix_index: int, bitmap: 'Bitmap') -> bool:
        ...
        if self._is_mock_mode:
            logger.debug(
                f"Mock mode: render_frame(matrix_index={matrix_index})")
            return True
        ...
        try:
            ...
            for y in range(8):
                for x in range(8):
                    ...
                    # For more control, modify this to use RGB values from
                    # bitmap
                    if self.matrix:
                        self.matrix.SetPixel(
                            col, y, pixel_value, pixel_value, pixel_value)

            return True

        except Exception as e:
            logger.error(
                f"Error rendering frame to matrix {matrix_index}: {e}")
            return False
