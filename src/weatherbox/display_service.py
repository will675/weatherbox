...
class WeatherDisplayService:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if forecast:
                logger.info(f"API connectivity verified: got {len(forecast)} days")
                self.retry_scheduler.record_success()
                self._last_forecast = forecast
            else:
                logger.warning("Initial API fetch failed; will retry on schedule")
    ...
    def run_cycle(self) -> bool:
        ...
        try:
            ...
            if not self.update_scheduler.should_update_now():
                ...
                logger.debug(f"Update window not ready; next in {wait_minutes}m")
                return True
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_forecast_count': len(self._last_forecast) if self._last_forecast else 0,
