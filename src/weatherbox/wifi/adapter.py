...
class WifiNetwork:
    ...
    def __init__(self, ssid: str, signal_strength: int, security: Optional[str] = None):
        """
        Args:
            ssid: Network SSID
    ...
    def __repr__(self) -> str:
        return f"WifiNetwork(ssid={self.ssid!r}, strength={self.signal_strength}, security={self.security!r})"


class WifiStatus:
    """Represents the current Wi-Fi connection status."""
    
    def __init__(self, connected: bool, ssid: Optional[str] = None, ip_address: Optional[str] = None):
    ...
    def __repr__(self) -> str:
        if self.connected:
            return f"WifiStatus(connected=True, ssid={self.ssid!r}, ip={self.ip_address!r})"
